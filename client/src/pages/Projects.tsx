...
export default function Projects() {
  ...
  if (isLoading || !projects || !sections) {
    return (
  ...
  // Group projects by section, only show sections with projects
  const sectionsWithProjects = sections
    .map(s => ({
      ...s,
      projects: projects.filter(p => p.section === s.name),
    }))
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="section-container py-12">
        {sectionsWithProjects.map((section, si) => (
          <motion.div
            key={section.id}
          ...
          >
            <h2 className="text-2xl font-heading font-bold text-foreground mb-6 flex items-center gap-3">
              ...
              {section.name}
              <span className="text-sm text-muted-foreground font-normal">({section.projects.length})</span>
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
              {section.projects.map((project, i) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.4, delay: si * 0.1 + i * 0.04 }}
